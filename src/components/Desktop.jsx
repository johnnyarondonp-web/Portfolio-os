...
// Grid cell size — must match icon container (w-24 = 96px)
const CELL_W = 100;
const CELL_H = 110;
const PADDING_X = 24;
const PADDING_Y = 24;
const COLS = 2; // max columns before wrapping right

/** Convert a flat index to a grid { x, y } position */
const indexToGrid = (index) => {
  const row = index % 5;        // column-major: fill rows first per column
...
/** Snap any { x, y } to the nearest grid cell */
const snapToGrid = (x, y, desktopW, desktopH) => {
  const col = Math.round((x - PADDING_X) / CELL_W);
  const row = Math.round((y - PADDING_Y) / CELL_H);

  const maxCol = Math.floor((desktopW - PADDING_X - 96) / CELL_W);
  const maxRow = Math.floor((desktopH - PADDING_Y - 96) / CELL_H);

  const clampedCol = Math.max(0, Math.min(col, maxCol));
  const clampedRow = Math.max(0, Math.min(row, maxRow));

  return {
    x: PADDING_X + clampedCol * CELL_W,
    y: PADDING_Y + clampedRow * CELL_H,
  };
};
...
const Desktop = ({ children, isDarkMode }) => {
  ...
  const [positions, setPositions] = useState(() => {
    const saved = localStorage.getItem('desktop_icon_positions_v2');
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        const allPresent = appsRegistry.every(app => parsed[app.id] !== undefined);
    ...
    appsRegistry.forEach((app, index) => {
      initial[app.id] = indexToGrid(index);
    });
    return initial;
  });
  ...
  const handleDragEnd = useCallback((appId, point) => {
    ...
    // Convert pointer coordinates to desktop-local coordinates
    const localX = point.x - rect.left;
    const localY = point.y - rect.top;

    const snapped = snapToGrid(localX, localY, rect.width, rect.height - 80);

    setPositions(prev => {
      const updated = { ...prev, [appId]: snapped };
      localStorage.setItem('desktop_icon_positions_v2', JSON.stringify(updated));
      return updated;
    });
  }, []);
      ...
      <div className="absolute top-0 left-0 w-full h-[calc(100vh-80px)] pointer-events-none z-10">
        {appsRegistry.map((app) => (
          <DesktopIcon
            ...
            position={positions[app.id] || indexToGrid(0)}
