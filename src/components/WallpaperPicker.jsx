...
// 3 temas × 2 modos (light + dark) = 6 wallpapers
...
// Agrupa los wallpapers en pares [light, dark] por tema
const THEMES = ['aurora', 'mountain', 'forest'];
...
const WallpaperPicker = ({ isOpen, onClose, currentWallpaperId, onSelectWallpaper, isDarkMode }) => {
  ...
  return (
    <>
      {/* Overlay */}
      <div
        className="fixed inset-0 z-[9800] bg-black/20 backdrop-blur-sm"
        onClick={onClose}
      />
      ...
      <div
        className={`
          fixed bottom-24 right-5 z-[9900] w-80 rounded-2xl shadow-2xl
                  ...
                  {pair.map((wp) => {
                    ...
                    return (
                      <button
                        ...
                        className={`
                          relative rounded-xl overflow-hidden aspect-video border-2 transition-all duration-200
                      ...
                      >
                        ...
                        <span
                          className={`
                            absolute top-1 left-1 px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wide
                            ${wp.mode === 'dark'
