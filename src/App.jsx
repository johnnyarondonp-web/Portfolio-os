...
function App() {
  ...
  const [bootStatus, setBootStatus] = useState('loggingIn'); // 'booting' | 'loggingIn' | 'ready'
  ...
  useEffect(() => {
    if (bootStatus === 'loggingIn') {
      const timer = setTimeout(() => {
        setBootStatus('ready');
      }, 400); // 400ms login blur screen, then fades to desktop
      return () => clearTimeout(timer);
    }
  }, [bootStatus]);

  // Global listener for sound trick
  useEffect(() => {
    const handleFirstClick = () => {
      // Create a simple oscillator sound (Windows inspired chime)
      if (!audioContextRef.current) {
        try {
          ...
          const playChime = () => {
            // Rich Windows-style startup chime using a C Major 9 chord
            const frequencies = [523.25, 659.25, 783.99, 987.77, 1046.50]; // C5, E5, G5, B5, C6
            
            frequencies.forEach((freq, i) => {
              const osc = ctx.createOscillator();
              const gain = ctx.createGain();
              
              // Mix sine and triangle for a richer timbre
              osc.type = i % 2 === 0 ? 'sine' : 'triangle';
              osc.frequency.setValueAtTime(freq, ctx.currentTime);
              
              gain.gain.setValueAtTime(0, ctx.currentTime);
              // Staggered attack and decay for depth
  ...
  const handleShutdown = () => {
    ...
    // Simulate BIOS boot sequence, then go back to loggingIn
    setTimeout(() => {
      setBootStatus('loggingIn');
    }, 4500); // 4.5 seconds for BIOS text
  };
        ...
        {bootStatus !== 'booting' && (
          ...
          >
            ...
            <motion.div
              ...
              transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }} // very fluid
